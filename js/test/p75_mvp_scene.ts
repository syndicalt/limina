...
import { serializeBehaviorSpec, serializeEventSpec, type BehaviorSpec, type EventSpec } from "../src/behavior/behavior-spec.ts";
...
const ground = ok("ground", await reg.invoke("scene.createEntity", { shape: "plane", size: 50, position: [0, 0, 0], color: COLOR.ground }, at())).entity as string;
...
const body = ok("body", await reg.invoke("scene.createEntity", { shape: "box", size: 2, position: [0, 1, 0], color: COLOR.stone }, at())).entity as string;
ok("roof", await reg.invoke("scene.createMesh", {
  geometry: { version: 1, kind: "box", width: 2.4, height: 1.0, depth: 2.4 },
  position: [0, 2.5, 0], color: COLOR.wood, seed: 100, parent: body,
}, at()));
ok("door", await reg.invoke("scene.createEntity", { shape: "plane", size: 0.8, position: [0, 0.4, 1.05], color: COLOR.trim, parent: body }, at()));
...
const stamp = async (x: number, seed: number): Promise<string[]> => {
  const r = ok(`stamp@${x}`, await reg.invoke("scene.instantiateGroup", { name: "townhouse", position: [x, 0, 0], yaw: 0, seed }, at()));
  return r.entities as string[];
};
const instA = await stamp(8, 1);
const instB = await stamp(16, 2);
const instA2 = await stamp(24, 1);
assert(instA.length === 3 && instB.length === 3 && instA2.length === 3, `each stamp must create 3 entities (body/roof/door); got ${instA.length},${instB.length},${instA2.length}`);

// ── on-brief conformance: every building colour is a DD palette colour; an off-brief colour is not ──
const buildingColors = [ground, body].map((e) => world.entities.resolve(e)?.material?.color).filter((c): c is number => typeof c === "number");
assert(buildingColors.length === 2, "the ground + body must carry first-class material colour");
for (const c of buildingColors) {
  assert(nearestPaletteDist(c) < 1e-6, `built colour 0x${c.toString(16)} is off-brief (nearest DD palette dist ${nearestPaletteDist(c).toFixed(1)})`);
}
...
const concatPositions = (entities: readonly string[]): number[] => {
  ...
  for (const e of entities) {
    ...
    mesh?.traverse?.((o) => {
      const g = (o as { geometry?: { getAttribute?: (n: string) => { array?: ArrayLike<number> } | undefined } }).geometry;
...
const gA = concatPositions(instA), gB = concatPositions(instB), gA2 = concatPositions(instA2);
assert(gA.length > 0 && gA.length === gA2.length && gA.every((v, i) => v === gA2[i]), "same instance seed must yield IDENTICAL geometry (deterministic stamp)");
assert(gA.length === gB.length && gA.some((v, i) => v !== gB[i]), "different instance seeds must yield DIFFERENT geometry (genuine reseed variation)");

// ════════════════════════════════════════════════════════════════════════════════════════════════
// 2. LIVING WORLD (B1): an NPC with a recorded behaviour + a scripted event.
// ════════════════════════════════════════════════════════════════════════════════════════════════

const npc = ok("npc", await reg.invoke("scene.createEntity", { shape: "capsule", size: 1, position: [6, 1, 6], color: COLOR.skin, tags: ["npc"] }, at())).entity as string;
const patrol: BehaviorSpec = { version: 1, kind: "patrol", waypoints: [[6, 0, 6], [12, 0, 6], [12, 0, 12]], speed: 1.5 };
...
try {
  outcome = await applyAuthorCommandsIsolated(reg, world, batch, { sessionId: "ses_p75", defaultAgentId: "agt_p75", defaultPerms: BUILDER, tick: ++tick });
} catch { threw = true; }
assert(!threw, "an out-of-band command must NOT throw out of the isolated build batch");
assert(outcome !== undefined && outcome.failures.length === 1 && outcome.failures[0].index === 1, `exactly the 1 bad command (index 1) must be a contained failure; got ${JSON.stringify(outcome?.failures)}`);
assert(world.entities.ids().length === before + 2, `both VALID batch commands must still apply despite the bad one (expected +2, got +${world.entities.ids().length - before})`);

// ════════════════════════════════════════════════════════════════════════════════════════════════
// 4. SELF-SUFFICIENT SNAPSHOT — the whole scene reloads into a FRESH world with NO authoring replay.
// ════════════════════════════════════════════════════════════════════════════════════════════════

const snap = captureWorldSnapshot(world, { sessionId: "ses_p75", tick, snapshotSeq: recorder.commands.length, events: authEvents });
...
assert(freshWorld.entities.ids().length === world.entities.ids().length, `restored world must have the same entity count (${world.entities.ids().length}), got ${freshWorld.entities.ids().length}`);
assert(freshWorld.entities.resolve(npc)?.behavior?.kind === "patrol", "NPC behaviour must survive the snapshot restore (no replay)");
assert(freshEvents.size() === 1 && serializeEventSpec(freshEvents.get(evId)!) === serializeEventSpec(authEvents.get(evId)!), "the scripted event must survive the snapshot restore");
const restoredBodyColor = freshWorld.entities.resolve(body)?.material?.color;
assert(restoredBodyColor !== undefined && nearestPaletteDist(restoredBodyColor) < 1e-6, "a building's on-brief material must survive the snapshot restore");
...
const replay = await replayCommands(recorder.commands, {
  ...
  makeRegistry: (tr) => { const r = new SkillRegistry(tr as LiminaTracer); replayEvents = registerCoreSkills(r).behaviorSpec.events; return r; },
...
assert(replay.world.entities.ids().length === world.entities.ids().length, `replay must rebuild the same entity count (${world.entities.ids().length}), got ${replay.world.entities.ids().length}`);
assert(replay.world.entities.resolve(body)?.material?.color === world.entities.resolve(body)?.material?.color, "replay must rebuild the building's on-brief material");

ops.op_log(`[js] p75_mvp_scene OK: one agent-authored stream one-shots an on-brief scene — ground + a townhouse recipe stamped 3x with genuine per-seed roof variation (A1+A2+A3, ${entityCount} entities all within the Design Direction's palette), an NPC with a recorded patrol behaviour + a scripted region event (B1); an out-of-band command mid-build is CONTAINED (A0, good commands still apply); and the WHOLE scene survives a self-sufficient snapshot restore into a fresh world AND replays to identical behaviour/event/entity state.`);
