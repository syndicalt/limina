...
// ── 1. The downhill base lip lands on the surface (measured by an independent raycast) ───
...
// Gradient is +x (height rises with column/x); downhill is −x. Lip = base plane = instance y.
let checked = 0, maxFloatNoEmbed = 0, maxLipGap = 0;
for (let i = 0; i < embedded.length && checked < 12; i++) {
  ...
  // The un-embedded base sits at the CENTER surface, so its downhill lip floats by ~r·slope.
  const floatNoEmbed = b.y - surfD;          // base plane (b.y == surfC) minus surface downhill
  // The embedded base lip (e.y) lands ON the surface at the downhill edge.
  const lipGap = Math.abs(e.y - surfD);
  maxFloatNoEmbed = Math.max(maxFloatNoEmbed, floatNoEmbed);
  maxLipGap = Math.max(maxLipGap, lipGap);
  assert(Math.abs(floatNoEmbed - EMBED_R * SLOPE) < 0.12, `instance ${i}: un-embedded float ${floatNoEmbed.toFixed(3)} != expected r·slope ${(EMBED_R * SLOPE).toFixed(3)} (diagnosis check)`);
  assert(lipGap < 0.12, `instance ${i}: embedded downhill lip still floats (gap ${lipGap.toFixed(3)} from the surface)`);
  checked++;
}
...
// A very steep ramp (HK 0.5 → slope 4.0). Raw sink r·slope = 4.8 would bury the prop; the
// cap (≤ 1.5·r and ≤ 3.0 world units) holds it to a sane depth.
...
for (let i = 0; i < shoreEmbed.length; i++) {
  ...
  const wouldUnclamped = s.y - Math.min(EMBED_R * shoreSlope * 1.0, Math.min(EMBED_R * 1.5, 3.0));
  if (wouldUnclamped < WATER_FLOOR - 1e-6) wouldSubmerge++;
...
ops.op_log(
  `p11_prop_tether OK: on a ${SLOPE} slope, embedRadius ${EMBED_R} sinks each prop so its downhill base lip lands ON the surface ` +
  `(max residual lip gap ${maxLipGap.toFixed(3)} m vs un-embedded float up to ${maxFloatNoEmbed.toFixed(3)} m = r·slope), ${checked} geometry-checked; ` +
