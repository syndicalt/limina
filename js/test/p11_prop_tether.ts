...
const EMBED_R = 1.2;
const EMBED_K = 0.35; // the current PLANAR-SLOPE sink constant (mirrors asset-scatter.ts)
const TRUNK_BURY = EMBED_R * SLOPE * EMBED_K;       // = r·slope·K
const SKIRT_FLOAT = EMBED_R * SLOPE * (1 - EMBED_K); // = r·slope·(1−K) — what's left of the un-embedded float
...
// ── 1. PLANAR-SLOPE SINK (K=EMBED_K): the wide cone is split into trunk BURY (r·slope·K) and
// downhill-skirt FLOAT (r·slope·(1−K)). Y-only placement cannot ground the skirt without burying
// the trunk; K=0.35 biases toward LESS bury (trunk bury 0.35·r·slope) and MORE float (skirt float
// 0.65·r·slope) than the strict-minmax K=0.5 — measured to read better on a uniform dome where
// the K=0.5 bury was a visible "all trees sunk a little." Measured against an independent raycast.
...
let checked = 0, maxFloatNoEmbed = 0;
for (let i = 0; i < embedded.length && checked < 12; i++) {
  ...
  const trunkBury = b.y - e.y;               // surfC − e.y  (== sink == r·slope·K)
  const skirtFloat = e.y - surfD;            // embedded base lip above the downhill surface (== r·slope·(1−K))
  ...
  // The K-biased split: trunk bury ≈ r·slope·K (trunkBury is analytic → tight).
  assert(Math.abs(trunkBury - TRUNK_BURY) < 0.03, `instance ${i}: trunk bury ${trunkBury.toFixed(3)} != r·slope·K ${TRUNK_BURY.toFixed(3)} (EMBED_K changed without updating this test)`);
  // And skirt float ≈ r·slope·(1−K) (via the independent raycast).
  assert(Math.abs(skirtFloat - SKIRT_FLOAT) < 0.08, `instance ${i}: skirt float ${skirtFloat.toFixed(3)} != r·slope·(1−K) ${SKIRT_FLOAT.toFixed(3)} (K split broken)`);
  // Both sides strictly less than the un-embedded float (the sink did real work).
...
// A very steep ramp (HK 0.5 → slope 4.0). Raw sink r·slope·K = 1.2·4·0.35 = 1.68 would still
...
ops.op_log(
  `p11_prop_tether OK: PLANAR-SLOPE SINK (K=${EMBED_K}) on a ${SLOPE} slope — trunk bury ${TRUNK_BURY.toFixed(3)} m (= r·slope·K), downhill skirt float ${SKIRT_FLOAT.toFixed(3)} m (= r·slope·(1−K)); ` +
  `un-embedded float up to ${maxFloatNoEmbed.toFixed(3)} m (= r·slope); both sides strictly < the un-embedded float; ${checked} geometry-checked; ` +
