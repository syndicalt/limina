...
import { WATER_OWNED_NODES_KEY } from "../src/render/water/material.ts";
...
const ownedNode = { dispose() {} };
ownedMaterial.userData[WATER_OWNED_NODES_KEY] = [ownedNode];
...
const counts = { geometry: 0, material: 0, texture: 0, node: 0, background: 0, override: 0, light: 0, hosted: 0, errors: 0 };
...
assert(counts.node === 1, "material-owned viewport/reflector node escaped world disposal");
...
console.log("p_render_browser_host_lifecycle OK: world geometry/material/texture/node/light resources dispose once while host cache resources survive");
