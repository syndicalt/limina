...
function freshCatalog(session: string): {
  registry: SkillRegistry;
  at: (tick: number) => Parameters<SkillRegistry["invoke"]>[2];
} {
...
// 2. catalog.publish of a new entry -> asset.catalog now returns 5, with the new entry's fields intact.
{
  ...
  assert(
    (pub.result as { count: number }).count === seedCount + 1,
    `catalog.publish count must be seed+1 (got ${(pub.result as { count: number }).count})`,
  );
  ...
  assert(
    well!.title === STONE_WELL.title && well!.category === STONE_WELL.category,
    "published entry's fields must be intact",
  );
  assert(well!.authoredBy === "claude-test-model", "authoredBy provenance must survive publish → catalog");
  assert(
    well!.boundsM[0] === STONE_WELL.boundsM[0] && well!.boundsM[2] === STONE_WELL.boundsM[2],
    "published entry's boundsM must be intact",
  );
  assert(
    Array.isArray(well!.qcTurntable) &&
      well!.qcTurntable.length === 2 &&
      well!.qcTurntable[0] === "qc/gate-test-well-a0.png",
...
// 3. Idempotency: publishing the same id again with a changed title -> still 5 entries, title updated.
{
  ...
  assert(
    (rep.result as { count: number }).count === seedCount + 1,
    `re-publish must NOT duplicate (got ${(rep.result as { count: number }).count})`,
  );

  const entries = await readCatalog(registry, at, 3);
  assert(
    entries.length === seedCount + 1,
    `catalog must still have seed+1 entries after re-publish (got ${entries.length})`,
  );
  const matches = entries.filter((e) => e.id === "gate-test-well.glb");
  assert(
    matches.length === 1,
    `re-publishing must replace, not duplicate (found ${matches.length} entries for gate-test-well.glb)`,
  );
...
//    it), asset.catalog declares a ".read" one (so the review gate never holds it — mirrors worldlog.tail).
{
  ...
  assert(
    catalogDef !== undefined && publishDef !== undefined,
    "both asset.catalog and catalog.publish must be registered",
  );
  assert(
    catalogDef!.permissions.every((p) => p.endsWith(".read")),
    `asset.catalog must declare only .read permissions (got ${catalogDef!.permissions.join(", ")})`,
  );
  assert(
    publishDef!.permissions.some((p) => !p.endsWith(".read")),
    `catalog.publish must declare a non-.read permission so the review gate holds it (got ${publishDef!.permissions.join(", ")})`,
  );
}

// Generic catalog authoring is prop-only. Enterable buildings have a separate functional,
// compiler-bound pipeline and must not silently fall back to asset.place/catalog.publish.
{
  const lane = freshCatalog("ses_catalog_functional_building_quarantine");
  const building = { ...STONE_WELL, id: "forbidden-house.glb", category: "dwelling" as const };
  const publish = await lane.registry.invoke("catalog.publish", building, lane.at(1));
  assert(
    !publish.success && /BuildingProgram/.test(publish.error?.message ?? ""),
    "generic catalog.publish accepted an enterable building",
  );
  const request = await lane.registry.invoke(
    "asset.request",
    { description: "An enterable timber house", category: "dwelling" },
    lane.at(2),
  );
  assert(
    !request.success && /BuildingProgram/.test(request.error?.message ?? ""),
    "generic asset.request accepted an enterable building",
  );
}
...
//    lists them in order; the same request sequence in a fresh state reproduces identical records.
{
  ...
  const r1 = await seqA.registry.invoke(
    "asset.request",
    { description: "A stone village well with a timber winch", category: "prop" },
    seqA.at(7),
  );
  assert(r1.success, `asset.request must succeed: ${JSON.stringify(r1.error)}`);
  assert(
    (r1.result as { requestId: string }).requestId === "req_7_0",
    `requestId must be deterministic tick+ordinal (got ${(r1.result as { requestId: string }).requestId})`,
  );
  ...
  assert(
    reqsA.length === 2 && reqsA[0].requestId === "req_7_0" && reqsA[1].requestId === "req_9_1",
    `requests must list in order with deterministic ids (got ${reqsA.map((r) => r.requestId).join(", ")})`,
  );

  const seqB = freshCatalog("ses_request_b");
  await seqB.registry.invoke(
    "asset.request",
    { description: "A stone village well with a timber winch", category: "prop" },
    seqB.at(7),
  );
  ...
  assert(
    stripAgent((listB.result as { requests: unknown }).requests) === stripAgent(reqsA),
    "the same request sequence in a fresh state must reproduce byte-identical records",
  );
  ...
  assert(
    reqDef !== undefined && reqDef.permissions.some((p) => !p.endsWith(".read")),
    "asset.request must declare a non-.read permission",
  );
  assert(
    listDef !== undefined && listDef.permissions.every((p) => p.endsWith(".read")),
    "asset.requests must declare only .read permissions",
  );
}

ops.op_log(
  "[js] p_asset_catalog OK: asset.catalog browses the seed catalog (assets/catalog.json) merged with this-session " +
    "catalog.publish entries (published wins on id collision, idempotent upsert, deterministic seed-then-publish " +
    "ordering); catalog.publish declares scene.write (held under builder.review) while asset.catalog declares " +
    "catalog.read (never held); asset.request records ＋New build requests with deterministic replay-safe ids and " +
