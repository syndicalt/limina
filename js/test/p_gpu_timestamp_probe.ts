...
// This is an OPT-IN supervised hardware diagnostic: with no LIMINA_GPU_POWER_PREFERENCE
// it has not been configured for this run — an environmental gap in the swept glob, not
// a probe failure — so announce the skip and exit clean. A NON-EMPTY but invalid value
// is a misconfiguration and stays a hard FAIL. Once opted in, every downstream error
// (no adapter, missing feature, invalid pairs) is a real FAIL by design.
if (configuredPreference === "") {
  ops.op_log("__LIMINA_SKIP__ p_gpu_timestamp_probe: LIMINA_GPU_POWER_PREFERENCE unset — supervised GPU diagnostic not configured for this run");
} else {
