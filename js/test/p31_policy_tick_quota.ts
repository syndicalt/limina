...
// PolicyEngine.windowHits prunes by `nowTick - windowTicks` over the recorded tick, so a
...
//
// What this pins (`windowTicks` is measured in ticks by construction):
...
//      windowTicks apart are ALL allowed. A wall-clock quota (all fired within
...
//      (> windowTicks=100) would expire the same-tick hits and let the final call through,
...
//      40ms sleeps (~160ms total, > windowTicks=100) STILL deny the 4th -- real elapsed
...
function freshEngine(limit = LIMIT): PolicyEngine {
  ...
  return new PolicyEngine().setQuota({ cap: "act", perSession: true, limit, windowTicks: WINDOW });
}
...
assert(spread.every((a) => a === true),
  `C: calls spaced > windowTicks apart must ALL be allowed (a wall-clock window would deny the last); got ${JSON.stringify(spread)}`);
...
const runSlept = await runSeq(bunched, 40); // ~160ms real total, > windowTicks=100 (E)
...
assert(JSON.stringify(runSlept) === JSON.stringify(runInstant1),
  `E: injecting ~160ms of real sleeps (> windowTicks=100) changed the decisions ${JSON.stringify(runSlept)} -- the window is reading WALL-CLOCK, not ctx.tick`);
...
ops.op_log(
  "p31_policy_tick_quota OK: the sliding-window quota is driven by ctx.tick -- the (L+1)th same-tick call is denied (quota.exceeded); " +
    ...
    "(> windowTicks=100) -- a wall-clock window would diverge, proving no Date.now leak.",
);
