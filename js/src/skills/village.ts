...
function meshFromBuffers(
  buf: { positions: ArrayLike<number>; uvs: ArrayLike<number>; indices: number[] },
  mat: THREE.Material,
): THREE.Mesh {
...
export function registerVillageSkills(
  ...
  const build: SkillDefinition<z.infer<typeof buildInput>, z.infer<typeof buildOutput>> = {
    ...
    version: "2.0.0",
    description:
      "Lay a terrain-aware settlement onto an editable terrain layer by placing curated library GLB assets. " +
      "Reads the live heightfield, runs the shared deterministic layout planner (focal on the chosen ground, " +
      "cluster terraced below, edge building beyond), and invokes asset.place per building. Deterministic + " +
      "replay-safe: the world log records the direction + steering + seed + PINNED asset hashes, NEVER the " +
      "transforms, which replay recomputes. Returns the placed entities + computed placements.",
    category: "three",
    ...
    handler: async (input, ctx) => {
      ...
      for (const spec of input.steering.buildings) {
        ...
        if (r === undefined) {
          r = footprintRadius(assets, assetId);
          radiusCache.set(assetId, r);
        }
        for (let i = 0; i < count; i++) {
          radii.push(r);
          instOf.push({ kind: "glb", assetId });
        }
      }
      ...
      if (anchorsIn.length > 0) {
        ...
        for (const anchor of anchorsIn) {
          ...
          for (let i = 0; i < want; i++) {
            instanceIndices.push(range.start + claimed[listIndex]);
            claimed[listIndex]++;
          }
      ...
      const villageSeed =
        (input.seed ?? (hashStr(JSON.stringify({ d: input.direction, s: input.steering })) as number)) >>> 0;
      const nestedCtx = () => ({
        agentId: ctx.agentId,
        sessionId: ctx.sessionId,
        permissions: new Set<string>(PLACE_PERMS),
      ...
      for (const p of placements) {
        ...
        if (inst.kind === "brief") {
          ...
          if (!res.success)
            throw new Error(
              `village.build: architecture.building failed for kit '${p.role}': ${JSON.stringify(res.error)}`,
            );
          entities.push((res.result as { root: string }).root);
      ...
      const groundGeoms: Array<{
        buf: { positions: ArrayLike<number>; uvs: ArrayLike<number>; indices: number[] };
        kind: "earth" | "gravel" | "cobble";
      }> = [];
      ...
      const laneBuf =
        siting.lane === "none"
          ? null
          : (buildLaneGeometry(heightAt, placed) as {
      ...
      if (siting.yard === "lawn") {
        ...
        const elevMin = sampler.seaLevel - 5,
          elevMax = oy + hi + 12;
        ...
        const lawnBladeBudget = canRenderGrass
          ? Math.max(
              3,
        ...
        for (let lawnIndex = 0; lawnIndex < placements.length; lawnIndex++) {
          ...
          if (canRenderGrass) {
            const lawnPlacements = planGrassBlades(tile, {
              ...
              exclusions: [lawnExcl[lawnIndex]],
              inclusions: [lawnIncl[lawnIndex]],
            });
            const built = buildGrassInstancedMesh(
              lawnPlacements,
        ...
        }
        {
          ...
          if (canRenderGrass) {
            const lawnTint = buildGrassGroundTint(tile, {
              baseColor: GRASS_CLIMATES.summer.base,
              elevationMin: elevMin,
              elevationMax: elevMax,
              slopeMax: 4.0,
              exclusions: lawnExcl,
        ...
        const decoAssets: { id: string; weight: number }[] = siting.lawnVegetation.map((a) => ({
          id: a.id,
          weight: a.weight,
        }));
        if (decoAssets.length > 0) {
          ...
          for (const inst of scatterAssets(tile, decoConfig.seed, decoConfig)) {
            let l = byId.get(inst.assetId);
            if (l === undefined) {
              l = [];
              byId.set(inst.assetId, l);
            }
            l.push(inst);
          }
      ...
      for (let k = 0; k < placements.length; k++) {
        ...
        const clearR =
          siting.yard === "lawn"
            ? r + 4
            : k === 0 && (siting.yard === "earth" || siting.yard === "cobble-courtyard")
              ? r + 16 + FOOTPRINT_TREE_MARGIN
              : r;
      ...
      if (laneCl !== null) {
        ...
        for (const s of laneCl.samples) {
          ...
          if (acc >= spacing) {
            exclusions.push({ x: s.x, z: s.z, r: laneR });
            acc = 0;
          }
          prev = s;
        }
          ...
          for (const clear of vegetationClears.get(undoTerrainId) ?? []) {
            try {
              ...
              if (r instanceof Promise)
                r.catch(() => {
                  /* render-only refresh; unwind already restored sim state */
                });
            } catch {
              /* render-only refresh */
            }
