...
import {
  mountFurniturePackReview,
  validateFurniturePackReviewAuthority,
} from "../render/furniture-pack-review-scene.ts";
...
const decoder = new TextDecoder("utf-8", { fatal: true }),
  ...
  authority = validateFurniturePackReviewAuthority(JSON.parse(decoder.decode(authorityBytes)));
const exact = (path: string, expected: string, label: string) => {
  const bytes = ops.op_read_asset(path);
  if (`sha256:${sha256(bytes)}` !== expected) throw new Error(`${label} hash drifted`);
  return bytes;
};
const evidenceBytes = exact(authority.pack.evidencePath, authority.pack.evidenceSha256, "furniture evidence"),
  evidence = JSON.parse(decoder.decode(evidenceBytes));
...
if (
  ...
  evidence.primitiveCount !== authority.pack.primitiveCount
)
  throw new Error("furniture authority no longer matches build evidence");
const [width, height] = authority.presentation.minimumResolution,
  engine = await createEngine({ width, height, gpuTimestampMode: "disabled", renderBaseline: false });
if (isSoftwareAdapter(engine.gpuAdapter))
  throw new Error(`furniture production review resolved a software adapter: ${JSON.stringify(engine.gpuAdapter)}`);
...
try {
  ...
  const lifecycleBaseline = world.entities.ids().length;
  mounted = await mountFurniturePackReview(world, authority);
  renderSyncSystem(world.ecs);
  const functionalMount = mounted as typeof mounted & {
    functionalEvidence: unknown;
    setReviewState(view: (typeof authority.evidenceViews)[number]): string;
  };
  if (
    typeof functionalMount.setReviewState !== "function" ||
    JSON.stringify(functionalMount.functionalEvidence) !== JSON.stringify(authority.functionalEvidence)
  )
    throw new Error("furniture mount lacks exact functional socket/collision/interaction evidence");
  const subject = mounted.entity,
    authoritativeBounds = mounted.authoritativeBounds,
  ...
  const captures = await withFrozenRendererTime(
    ...
    async (beginFrame) => {
      ...
      for (let i = 0; i < authority.evidenceViews.length; i++) {
        const view = authority.evidenceViews[i],
          appliedState = functionalMount.setReviewState(view),
          reviewState = { type: view.type, state: view.state, appliedState };
        mounted!.setScaleProxyVisible(view.id === "front");
        ...
        for (let frame = 0; frame < (i === 0 ? authority.presentation.warmupFrames : 2); frame++) {
          beginFrame();
          renderer.render(engine.scene, engine.camera);
          ops.op_surface_present(engine.context);
        }
        mounted!.setSubjectVisible(false);
        let baseline;
        try {
        ...
        const captured = await withPresentedNativeSurfaceFrame(
          ...
          async () => {
            ...
            const cpuEncodeMs = Number((performance.now() - started).toFixed(3)),
              ...
              readback = await readNativeSurfaceRgba({
            ...
            return { cpuEncodeMs, submission, paired, resources, readback };
          },
        );
        out.push(
          Object.freeze({
            ...
            width: captured.readback.width,
            height: captured.readback.height,
            surfaceFormat: captured.readback.format,
            rgbaContentHash: portableAssetContentHash(captured.readback.rgba),
            rgbaByteLength: captured.readback.rgba.byteLength,
            rgbaBase64: bytesToBase64(captured.readback.rgba),
  ...
  mounted = undefined;
  const afterDispose = world.entities.ids().length;
  if (afterDispose !== lifecycleBaseline)
    throw new Error(`furniture review lifecycle leaked entities: ${lifecycleBaseline} -> ${afterDispose}`);
  ops.op_write_trace(
    TRACE_NAME,
    JSON.stringify({
      ...
      mounted: {
        ...
        authoritativeBounds,
        stageEntities,
        functionalEvidence: functionalMount.functionalEvidence,
        collisionEvidence: "compound-semantic-functional-placement",
      },
      lifecycle: { baselineEntities: lifecycleBaseline, afterDisposeEntities: afterDispose },
      captures,
    }) + "\n",
  );
  ...
  if (failures.length)
    captureFailure = new AggregateError(
      captureFailure === undefined ? failures : [captureFailure, ...failures],
      "furniture capture teardown failed",
    );
}
