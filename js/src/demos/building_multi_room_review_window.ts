...
const authorityPath = ops.op_read_env(FB4_REVIEW_AUTHORITY_ENV),
  traceName = ops.op_read_env(FB4_REVIEW_TRACE_ENV);
...
if (
  ...
  throw new Error(`${FB4_REVIEW_TRACE_ENV} must be a bare .json filename`);
const decoder = new TextDecoder("utf8", { fatal: true }),
  authorityBytes = ops.op_read_asset(authorityPath),
  authority = assertMultiRoomReviewCaptureReady(
    validateMultiRoomReviewAuthority(JSON.parse(decoder.decode(authorityBytes))),
  );
verifyMultiRoomReviewV4Closure(authority, (path) => ops.op_read_asset(path));
const glb = ops.op_read_asset(authority.candidate.glb.path),
  contract = parseFunctionalBuildingContract(glb),
...
const site = resolveFunctionalBuildingSitePlacement({
  contract,
...
const runtimePackBytes = ops.op_read_asset(authority.siteReviewEnvelope.runtimePack.path),
  populationMaximumHorizontalReachM = verifySiteReviewRuntimePack(authority.siteReviewEnvelope, runtimePackBytes),
  discretePopulationHardExclusionAt = createSiteReviewDiscretePopulationExclusion({
  ...
  siteReviewCameraEvidence = verifySiteReviewCameras({
    envelope: authority.siteReviewEnvelope,
    position: authority.placement.position,
    yaw: authority.placement.yaw,
...
cache.configureKtx2(renderer);
let environment: Awaited<ReturnType<typeof mountTemperateFidelityScene>> | undefined,
  mounted: Awaited<ReturnType<typeof mountMultiRoomReview>> | undefined,
  failure: unknown;
...
try {
  if (isSoftwareAdapter(engine.gpuAdapter))
    throw new Error(`FB-4 review resolved a software adapter: ${JSON.stringify(engine.gpuAdapter)}`);
  ...
  environment = await mountTemperateFidelityScene({
    ...
    height,
    gltfCache: cache,
    populationHardExclusionAt: site.containsWorldXZ,
    discretePopulationHardExclusionAt,
  });
  ops.op_physics_create_world(0);
  const baseline = environment.world.entities.ids().length;
  mounted = await mountMultiRoomReview(environment.world, authority, site.rootWorldY);
    ...
    async (beginFrame) => {
      ...
      for (let index = 0; index < authority.evidenceViews.length; index++) {
        const view = authority.evidenceViews[index],
          doorPose = await mounted!.setReviewViewDoorPose(view.id);
        renderSyncSystem(environment!.world.ecs);
        camera.position.set(
          view.camera.position[0],
          view.camera.position[1] + site.rootWorldY,
          view.camera.position[2],
        );
        camera.lookAt(view.camera.target[0], view.camera.target[1] + site.rootWorldY, view.camera.target[2]);
        camera.fov = view.camera.fovDeg;
        ...
        const capture = await withPresentedNativeSurfaceFrame(
          () => ops.op_surface_present(engine.context),
          async () => {
            renderer.info.reset();
            beginFrame();
            ...
            if (submission.renderCalls <= 1 || submission.drawCalls <= 1 || submission.triangles <= 1)
              throw new Error(`FB-4 ${view.id} did not submit a whole production frame`);
            ...
            return { submission: { ...submission, cpuEncodeMs }, resources, pixels };
          },
        );
        output.push(
          Object.freeze({
            ...
            role: view.role,
            camera: view.camera,
            openDoorIds: doorPose.openDoorIds,
            width: capture.pixels.width,
            height: capture.pixels.height,
            surfaceFormat: capture.pixels.format,
            rgbaByteLength: capture.pixels.rgba.byteLength,
            rgbaContentHash: portableAssetContentHash(capture.pixels.rgba),
            rgbaBase64: b64(capture.pixels.rgba),
            renderSubmission: capture.submission,
            rendererResources: capture.resources,
          }),
        );
      }
  ...
  const functionalEvidence = {
    root: mounted.root,
    doors: mounted.doors.length,
    semanticDoorIds: mounted.semanticDoorIds,
    reviewDoorPosePlan: mounted.doorPosePlan,
    parts: mounted.parts.length,
    path: mounted.path,
    anchors: mounted.anchors.length,
  };
  await mounted.dispose();
  mounted = undefined;
  const after = environment.world.entities.ids().length;
  if (after !== baseline) throw new Error(`FB-4 review leaked entities: ${baseline} -> ${after}`);
  ops.op_write_trace(
    traceName,
    JSON.stringify({
      schema: "limina.fb4-multi-room-native-review-set/v1",
      ...
      site: {
        ...
        terrainRelief: site.terrainRelief,
        reviewEnvelope: { populationMaximumHorizontalReachM, cameraEvidence: siteReviewCameraEvidence },
      },
      functionalPlacement: functionalEvidence,
      lifecycle: { baselineEntities: baseline, afterDisposeEntities: after, disposed: true },
      captures,
    }) + "\n",
  );
} catch (error) {
  ...
  for (const operation of [
    async () => mounted?.dispose(),
    async () => environment?.dispose(),
    async () => cache.dispose(),
    async () => engine.disposeRenderBaseline(),
    async () => renderer.dispose(),
  ])
    try {
      await operation();
    } catch (error) {
  ...
  if (errors.length)
    failure = new AggregateError(failure === undefined ? errors : [failure, ...errors], "FB-4 capture teardown failed");
}
