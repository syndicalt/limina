...
export interface FunctionalBuildingVerticalLink {
  ...
  flights?: { from: V3; to: V3; riserCount: number }[];
  intermediateLandings?: { center: V3; halfExtents: V2; yawRadians: number }[];
  approaches?: {
...
export interface FunctionalBuildingVisibilityCell {
  id: string;
  roomIds: string[];
  nodeIds: string[];
}
...
function object(value: unknown, label: string): JsonObject {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new Error(`functional building: ${label} must be an object`);
  return value as JsonObject;
}
function string(value: unknown, label: string): string {
  if (typeof value !== "string" || value.trim() === "")
    throw new Error(`functional building: ${label} must be a non-empty string`);
  return value;
}
...
function finite(value: unknown, label: string): number {
  if (typeof value !== "number" || !Number.isFinite(value))
    throw new Error(`functional building: ${label} must be finite`);
  return value;
}
...
function quaternion(value: unknown, label: string): [number, number, number, number] {
  if (!Array.isArray(value) || value.length !== 4)
    throw new Error(`functional building: ${label} must be a quaternion`);
  ...
  if (Math.abs(Math.sqrt(out.reduce((sum, axis) => sum + axis * axis, 0)) - 1) > 1e-5)
    throw new Error(`functional building: ${label} must be normalized`);
  return out;
}
function stringArray(value: unknown, label: string): string[] {
  if (!Array.isArray(value) || value.length === 0)
    throw new Error(`functional building: ${label} must be a non-empty array`);
...
function exactKeys(value: JsonObject, required: readonly string[], optional: readonly string[], label: string): void {
  ...
  for (const key of Object.keys(value))
    if (!allowed.has(key)) throw new Error(`functional building: ${label}.${key} is unsupported`);
}
function boundedArray(value: unknown, label: string, minimum: number, maximum: number): unknown[] {
  if (!Array.isArray(value) || value.length < minimum || value.length > maximum)
    throw new Error(`functional building: ${label} must contain ${minimum}..${maximum} entries`);
  return value;
}
...
function gltfJson(bytes: Uint8Array): JsonObject {
  ...
  try {
    return object(JSON.parse(new TextDecoder().decode(jsonBytes).trim()), "glTF root");
  } catch (error) {
    throw new Error(
      `functional building: invalid glTF JSON: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}

/** Parse and strictly validate semantic authority embedded in glTF asset/node extras. */
export function parseFunctionalBuildingContract(
  bytes: Uint8Array,
): FunctionalBuildingContract | FunctionalBuildingContractV2 {
  const json = gltfJson(bytes);
  const asset = object(json.asset, "asset");
  const authority = object(
    object(asset.extras, "asset.extras").liminaFunctionalBuilding,
  ...
  if (authority.site !== undefined) {
    ...
    if (
      terrainClearance < 0.05 ||
      terrainClearance > 1 ||
      vegetationClearance < 0 ||
      vegetationClearance > 5 ||
      maximumTerrainRelief <= 0 ||
      maximumTerrainRelief > 5
    )
      throw new Error("functional building: site policy is outside bounded construction limits");
    site = {
      footprintCenter: vec2(rawSite.footprintCenter, "site.footprintCenter"),
  ...
  const scenes = Array.isArray(json.scenes) ? json.scenes : [],
    sceneIndex = json.scene === undefined ? 0 : finite(json.scene, "scene");
  ...
  const scene = object(scenes[sceneIndex], `scenes[${sceneIndex}]`),
    sceneRoots = scene.nodes;
  ...
  const visit = (index: number): void => {
    ...
    const raw = object(nodes[index], `nodes[${index}]`);
    if (raw.children !== undefined) {
      if (!Array.isArray(raw.children))
        throw new Error(`functional building: nodes[${index}].children must be an array`);
      for (const child of raw.children) visit(finite(child, `nodes[${index}].children`));
    }
  ...
  for (const item of semantic.values()) {
    if (item.role === "collider") {
      ...
      colliders.push({
        id: item.nodeId,
        nodeId: item.nodeId,
        halfExtents: vec3(item.data.halfExtents, `${item.nodeId}.halfExtents`, true),
        center: vec3(item.data.center, `${item.nodeId}.center`),
        ...(item.data.rotation === undefined
          ? {}
          : { rotation: quaternion(item.data.rotation, `${item.nodeId}.rotation`) }),
      });
      ...
      doors.push({
        ...
        roomId,
        portalId,
        hinge: vec3(item.data.hinge, `${item.nodeId}.hinge`),
        closedYaw,
        openYaw,
        halfExtents: vec3(item.data.halfExtents, `${item.nodeId}.halfExtents`, true),
        center: vec3(item.data.center, `${item.nodeId}.center`),
  ...
  if (site?.entranceSupport) {
    const support = site.entranceSupport,
      item = semantic.get(support.sourcePrimitiveId),
      boxData = item?.data.box === undefined ? undefined : object(item.data.box, `${support.sourcePrimitiveId}.box`);
    if (item?.role !== "architecture-primitive" || boxData === undefined)
      throw new Error("functional building: entrance support source primitive is unresolved");
    const center = vec3(boxData.center, `${support.sourcePrimitiveId}.box.center`),
      halfExtents = vec3(boxData.halfExtents, `${support.sourcePrimitiveId}.box.halfExtents`, true),
      yaw = finite(boxData.yawRadians, `${support.sourcePrimitiveId}.box.yawRadians`);
    if (
      Math.abs(center[0] - support.center[0]) > 0.001 ||
      Math.abs(center[2] - support.center[1]) > 0.001 ||
      Math.abs(halfExtents[0] - support.halfExtents[0]) > 0.001 ||
      Math.abs(halfExtents[2] - support.halfExtents[1]) > 0.001 ||
      Math.abs(yaw - support.yawRadians) > 0.001 ||
      Math.abs(center[1] - halfExtents[1] - (support.exteriorGradeY - support.bearingDepth)) > 0.001
    )
      throw new Error("functional building: entrance support does not match its structural source primitive");
    const c = Math.cos(support.yawRadians),
      s = Math.sin(support.yawRadians),
      limitX = site.footprintHalfExtents[0] + site.vegetationClearance,
      limitZ = site.footprintHalfExtents[1] + site.vegetationClearance;
    for (const sx of [-support.halfExtents[0], support.halfExtents[0]])
      for (const sz of [-support.halfExtents[1], support.halfExtents[1]]) {
        const x = support.center[0] + sx * c + sz * s - site.footprintCenter[0],
          z = support.center[1] - sx * s + sz * c - site.footprintCenter[1];
        if (Math.abs(x) > limitX + 1e-9 || Math.abs(z) > limitZ + 1e-9)
  ...
  for (const door of doors) {
    ...
    const channels = Array.isArray(object(clip, `${door.id} clip`).channels)
      ? (object(clip, `${door.id} clip`).channels as unknown[])
      : [];
  ...
  const rooms = boundedArray(authority.rooms, "rooms", 1, 32).map((raw, index): FunctionalBuildingRoom => {
    ...
    const id = stableId(value.id, `${label}.id`),
      boundsRaw = object(value.bounds, `${label}.bounds`);
    exactKeys(boundsRaw, ["center", "halfExtents"], [], `${label}.bounds`);
    const bounds = {
      center: vec3(boundsRaw.center, `${label}.bounds.center`),
      halfExtents: vec3(boundsRaw.halfExtents, `${label}.bounds.halfExtents`, true),
    };
    const finishedFloorY = finite(value.finishedFloorY, `${label}.finishedFloorY`),
      ceilingY = finite(value.ceilingY, `${label}.ceilingY`);
    const storey = finite(value.storey, `${label}.storey`);
    if (!Number.isSafeInteger(storey) || storey < 0 || storey > 63)
      throw new Error(`functional building: ${label}.storey must be an integer within [0,63]`);
    if (
      ceilingY - finishedFloorY < 1.8 ||
      Math.abs(bounds.center[1] - bounds.halfExtents[1] - finishedFloorY) > 1e-4 ||
      Math.abs(bounds.center[1] + bounds.halfExtents[1] - ceilingY) > 1e-4
    )
      throw new Error(
        `functional building: ${label} bounds must span its finished floor and ceiling with at least 1.8m headroom`,
      );
    ...
    return {
      ...
      ceilingY,
      storey,
      visibilityCellId: stableId(value.visibilityCellId, `${label}.visibilityCellId`),
      acoustics: {
        absorption: coefficient(acousticsRaw.absorption, `${label}.acoustics.absorption`),
        reverb: coefficient(acousticsRaw.reverb, `${label}.acoustics.reverb`),
      },
    };
  });
  const roomMap = new Map(rooms.map((room) => [room.id, room]));
  if (roomMap.size !== rooms.length || roomIds.length !== rooms.length || roomIds.some((id) => !roomMap.has(id)))
  ...
  const portals = boundedArray(authority.portals, "portals", 1, 64).map((raw, index): FunctionalBuildingPortal => {
    ...
    const id = stableId(value.id, `${label}.id`),
      kind = string(value.kind, `${label}.kind`);
    ...
    if (!Array.isArray(value.roomIds) || value.roomIds.length !== 2)
      throw new Error(`functional building: ${label}.roomIds must be a pair`);
    const endpoints = value.roomIds.map((entry, endpoint) =>
      entry === null ? null : string(entry, `${label}.roomIds[${endpoint}]`),
    ) as [string | null, string | null];
    if (
      (endpoints[0] === null && endpoints[1] === null) ||
      endpoints[0] === endpoints[1] ||
      endpoints.some((entry) => entry !== null && !roomMap.has(entry))
    )
      throw new Error(`functional building: ${label}.roomIds has unresolved or degenerate endpoints`);
    if (typeof value.exterior !== "boolean" || value.exterior !== endpoints.includes(null))
      throw new Error(`functional building: ${label}.exterior must exactly match its null exterior endpoint`);
    const center = vec3(value.center, `${label}.center`),
      halfExtents = vec3(value.halfExtents, `${label}.halfExtents`, true);
        ...
        if (
          !center.every(
            (axis, dimension) =>
              ...
              room.bounds.halfExtents[dimension] + halfExtents[dimension] + 1e-6,
          )
        )
          throw new Error(`functional building: ${label} does not touch room ${endpoint}`);
      }
    const doorId = value.doorId === undefined ? undefined : stableId(value.doorId, `${label}.doorId`);
    if ((kind === "door") !== (doorId !== undefined))
      throw new Error(`functional building: ${label} door portals require doorId and passages prohibit it`);
    return {
      id,
      kind,
      ...
      center,
      halfExtents,
      acousticTransmission: coefficient(value.acousticTransmission, `${label}.acousticTransmission`),
      ...(doorId === undefined ? {} : { doorId }),
    };
  });
  const portalMap = new Map(portals.map((portal) => [portal.id, portal]));
  if (
    portalMap.size !== portals.length ||
    portalIds.length !== portals.length ||
    portalIds.some((id) => !portalMap.has(id))
  )
  ...
  const verticalLinks = boundedArray(authority.verticalLinks, "verticalLinks", 0, 32).map(
    (raw, index): FunctionalBuildingVerticalLink => {
      const label = `verticalLinks[${index}]`,
        value = object(raw, label);
      exactKeys(
        value,
        [
      ...
      const fromRoomId = string(value.fromRoomId, `${label}.fromRoomId`),
        toRoomId = string(value.toRoomId, `${label}.toRoomId`),
        from = vec3(value.from, `${label}.from`),
        to = vec3(value.to, `${label}.to`);
      const fromRoom = roomMap.get(fromRoomId),
        toRoom = roomMap.get(toRoomId);
      if (!fromRoom || !toRoom || fromRoom === toRoom || !contains(fromRoom, from) || !contains(toRoom, to))
        throw new Error(`functional building: ${label} endpoints must resolve inside two distinct rooms`);
      const rise = positive(value.rise, `${label}.rise`),
        run = positive(value.run, `${label}.run`),
        actualRise = Math.abs(to[1] - from[1]);
      const riserCount = finite(value.riserCount, `${label}.riserCount`),
        treadDepth = positive(value.treadDepth, `${label}.treadDepth`);
      const clearWidth = positive(value.clearWidth, `${label}.clearWidth`),
        clearHeight = positive(value.clearHeight, `${label}.clearHeight`);
      ...
      const upperFloorOpening = {
        center: vec2(openingRaw.center, `${label}.upperFloorOpening.center`),
        halfExtents: vec2(openingRaw.halfExtents, `${label}.upperFloorOpening.halfExtents`, true),
      };
      const flights =
          value.flights === undefined
            ? undefined
            : boundedArray(value.flights, `${label}.flights`, 2, 4).map((rawFlight, flightIndex) => {
                const flight = object(rawFlight, `${label}.flights[${flightIndex}]`);
                exactKeys(flight, ["from", "to", "riserCount"], [], `${label}.flights[${flightIndex}]`);
                return {
                  from: vec3(flight.from, `${label}.flights[${flightIndex}].from`),
                  to: vec3(flight.to, `${label}.flights[${flightIndex}].to`),
                  riserCount: finite(flight.riserCount, `${label}.flights[${flightIndex}].riserCount`),
                };
              }),
        intermediateLandings =
          value.intermediateLandings === undefined
            ? undefined
            : boundedArray(value.intermediateLandings, `${label}.intermediateLandings`, 1, 3).map(
                (rawLanding, landingIndex) => {
                  const landing = object(rawLanding, `${label}.intermediateLandings[${landingIndex}]`);
                  exactKeys(
                    landing,
                    ["center", "halfExtents", "yawRadians"],
                    [],
                    `${label}.intermediateLandings[${landingIndex}]`,
                  );
                  return {
                    center: vec3(landing.center, `${label}.intermediateLandings[${landingIndex}].center`),
                    halfExtents: vec2(
                      landing.halfExtents,
                      `${label}.intermediateLandings[${landingIndex}].halfExtents`,
                      true,
                    ),
                    yawRadians: finite(landing.yawRadians, `${label}.intermediateLandings[${landingIndex}].yawRadians`),
                  };
                },
              );
      const parseApproach = (raw: unknown, socket: "bottom" | "top") => {
          const approach = object(raw, `${label}.approaches.${socket}`);
          exactKeys(approach, ["center", "direction", "halfExtents"], [], `${label}.approaches.${socket}`);
          return {
            center: vec3(approach.center, `${label}.approaches.${socket}.center`),
            direction: vec3(approach.direction, `${label}.approaches.${socket}.direction`),
            halfExtents: vec2(approach.halfExtents, `${label}.approaches.${socket}.halfExtents`, true),
          };
        },
        approachesRaw = value.approaches === undefined ? undefined : object(value.approaches, `${label}.approaches`),
        approaches =
          approachesRaw &&
          (exactKeys(approachesRaw, ["bottom", "top"], [], `${label}.approaches`),
          { bottom: parseApproach(approachesRaw.bottom, "bottom"), top: parseApproach(approachesRaw.top, "top") }),
        bottomLandingDepth =
          value.bottomLandingDepth === undefined
            ? undefined
            : positive(value.bottomLandingDepth, `${label}.bottomLandingDepth`),
        topLandingDepth =
          value.topLandingDepth === undefined ? undefined : positive(value.topLandingDepth, `${label}.topLandingDepth`),
        clearancePolicy =
          value.clearancePolicy === undefined ? undefined : string(value.clearancePolicy, `${label}.clearancePolicy`),
        dx = to[0] - from[0],
        dz = to[2] - from[2],
        actualRun = flights
          ? flights.reduce(
        ...
        multiValid =
          !flights ||
          (clearancePolicy === "structural-footprints-and-controller-sockets-v1" &&
            intermediateLandings?.length === flights.length - 1 &&
            approaches &&
            bottomLandingDepth &&
            topLandingDepth &&
            flights.reduce((sum, flight) => sum + flight.riserCount, 0) === riserCount &&
            flights.every((flight) => flight.to[1] > flight.from[1] && Number.isSafeInteger(flight.riserCount)));
      if (
        !Number.isSafeInteger(riserCount) ||
        riserCount < 2 ||
        riserCount > 64 ||
        rise / riserCount > 0.2 ||
        treadDepth < 0.25 ||
        ...
        clearWidth < 0.8 ||
        clearHeight < 1.9 ||
        Math.abs(run - treadDepth * riserCount) > 1e-4 ||
        Math.abs(actualRun - run) > 1e-4 ||
        (!flights && Math.abs(dx) > 1e-4 && Math.abs(dz) > 1e-4) ||
        !multiValid ||
        Math.abs(from[1] - fromRoom.finishedFloorY) > 1e-4 ||
        Math.abs(to[1] - toRoom.finishedFloorY) > 1e-4 ||
        toRoom.finishedFloorY <= fromRoom.finishedFloorY ||
        toRoom.storey <= fromRoom.storey ||
        ...
        upperFloorOpening.halfExtents[0] * 2 < clearWidth - 1e-6 ||
        upperFloorOpening.halfExtents[1] * 2 < clearWidth - 1e-6 ||
        Math.abs(upperFloorOpening.center[0] - toRoom.bounds.center[0]) + upperFloorOpening.halfExtents[0] >
          toRoom.bounds.halfExtents[0] + 1e-6 ||
        Math.abs(upperFloorOpening.center[1] - toRoom.bounds.center[2]) + upperFloorOpening.halfExtents[1] >
          toRoom.bounds.halfExtents[2] + 1e-6 ||
        Math.abs(rise - actualRise) > 1e-4 ||
        Math.abs(actualRise - Math.abs(toRoom.finishedFloorY - fromRoom.finishedFloorY)) > 1e-4
      )
  ...
  const spawnAnchors = boundedArray(authority.spawnAnchors, "spawnAnchors", 1, 256).map(
    (raw, index): FunctionalBuildingSpawnAnchor => {
      const label = `spawnAnchors[${index}]`,
        value = object(raw, label);
      exactKeys(
        value,
      ...
      const id = stableId(value.id, `${label}.id`),
        roomId = stableId(value.roomId, `${label}.roomId`),
        kind = string(value.kind, `${label}.kind`),
        position = vec3(value.position, `${label}.position`),
        direction = vec3(value.direction, `${label}.direction`),
        length = Math.hypot(...direction);
      if (kind !== "player" && kind !== "npc" && kind !== "item")
        throw new Error(`functional building: ${label}.kind is unsupported`);
      const clearanceRadius = positive(value.clearanceRadius, `${label}.clearanceRadius`),
        clearanceHeight = positive(value.clearanceHeight, `${label}.clearanceHeight`);
      ...
      if (
        ...
        clearanceHeight > 3 ||
        clearanceHeight < 0.1 ||
        position[1] < room.finishedFloorY - 1e-4 ||
        position[1] + clearanceHeight > room.ceilingY + 1e-4 ||
      ...
      if (Math.abs(length - 1) > 1e-4 || Math.abs(direction[1]) > 1e-4)
        throw new Error(`functional building: ${label}.direction must be normalized and horizontal`);
      return {
        id,
        roomId,
        kind: kind as "player" | "npc" | "item",
        position,
        direction,
        clearanceRadius,
        clearanceHeight,
  ...
  if (
    new Set(spawnAnchors.map((anchor) => anchor.id)).size !== spawnAnchors.length ||
    rooms.some((room) => !spawnAnchors.some((anchor) => anchor.roomId === room.id))
  )
  ...
  const visibilityCells = boundedArray(authority.visibilityCells, "visibilityCells", 1, 32).map(
    (raw, index): FunctionalBuildingVisibilityCell => {
      const label = `visibilityCells[${index}]`,
        value = object(raw, label);
      exactKeys(value, ["id", "roomIds", "nodeIds"], [], label);
      const id = stableId(value.id, `${label}.id`),
        coveredRooms = stringArray(value.roomIds, `${label}.roomIds`),
        nodeIds = stringArray(value.nodeIds, `${label}.nodeIds`);
      if (coveredRooms.some((roomId) => !roomMap.has(roomId)) || nodeIds.some((nodeId) => !semantic.has(nodeId)))
        throw new Error(`functional building: ${label} contains unresolved room or node ids`);
  ...
  if (
    cellMap.size !== visibilityCells.length ||
    new Set(visibleNodeIds).size !== visibleNodeIds.length ||
    rooms.some((room) => !cellMap.get(room.visibilityCellId)?.roomIds.includes(room.id)) ||
    roomIds.some((roomId) => visibilityCells.filter((cell) => cell.roomIds.includes(roomId)).length !== 1)
  )
    throw new Error(
  ...
  const adjacency = new Map(roomIds.map((id) => [id, new Set<string>()])),
    exterior = new Set<string>();
  for (const portal of portals) {
    const [a, b] = portal.roomIds;
    if (a === null) exterior.add(b!);
    else if (b === null) exterior.add(a);
    else {
  ...
  for (const link of verticalLinks) {
    adjacency.get(link.fromRoomId)!.add(link.toRoomId);
    adjacency.get(link.toRoomId)!.add(link.fromRoomId);
  }
  if (exterior.size === 0) throw new Error("functional building: at least one exterior portal is required");
  const reached = new Set(exterior),
    queue = [...exterior];
  while (queue.length)
