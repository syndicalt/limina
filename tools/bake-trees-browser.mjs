...
const OUT_DIR = join(REPO_ROOT, "assets", "vegetation", "temperate-canopy");
...
const browser = await chromium.launch({ executablePath: chrome, headless: true, args: ["--no-sandbox", "--disable-dev-shm-usage", "--use-gl=angle", "--use-angle=swiftshader", "--disable-gpu"] });
try {
  ...
  const manifest = { generator: "ez-tree (browser, textured, software-rasterized)", species: {} };
  for (const g of glbs) {
