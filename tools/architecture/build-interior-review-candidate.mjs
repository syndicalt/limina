...
const portable = (root, path) => {
  ...
  if (value === ".." || value.startsWith("../") || isAbsolute(value))
    throw new Error(`I1 candidate input escapes repo root: ${path}`);
  return value;
};
...
export async function buildInteriorReviewCandidate({
  ...
  const root = resolve(repoRoot),
    ...
    draftAbsolute = resolve(root, draftPath),
    outputAbsolute = resolve(root, outputPath);
  const [authorityBytes, captureBytes, draftBytes] = await Promise.all([
      readFile(authorityAbsolute),
      readFile(captureAbsolute),
      readFile(draftAbsolute),
    ]),
    authority = validateStagedInteriorProxyReviewAuthority(JSON.parse(authorityBytes)),
    capture = JSON.parse(captureBytes),
    draft = validateBuildingStageArtifact(JSON.parse(draftBytes));
  const planBytes = await readFile(resolve(root, authority.plan.path)),
    plan = validateBuildingInteriorPlanV2(JSON.parse(planBytes));
  if (
    portable(root, resolve(root, authority.plan.path)) !== authority.plan.path ||
    sha(planBytes) !== authority.plan.sha256 ||
    portableAssetContentHash(planBytes) !== authority.plan.contentHash ||
    plan.planId !== authority.plan.planId ||
    plan.revision !== authority.plan.revision
  )
    throw new Error("I1 authority plan bytes drifted");
  if (
    portable(root, draftAbsolute) !== authority.stageArtifact.path ||
    draft.kind !== "interior-plan" ||
    draft.status !== "draft" ||
    draft.artifactId !== authority.stageArtifact.artifactId ||
    draft.revision !== authority.stageArtifact.revision ||
    draft.contractHash !== authority.plan.canonicalHash ||
    draft.contentHash !== authority.plan.sha256 ||
    sha(draftBytes) !== authority.stageArtifact.sha256 ||
    portableAssetContentHash(draftBytes) !== authority.stageArtifact.contentHash ||
    draft.evidence.length !== 0 ||
    draft.metadata?.gate !== "I1-layout" ||
    draft.metadata?.proxyOnly !== true
  )
    throw new Error("draft does not bind the exact corrected I1 authority and plan");
  if (
    capture.schema !== "limina.staged-interior-proxy-native-review-set/v1" ||
    ...
    capture.authority?.path !== portable(root, authorityAbsolute) ||
    capture.authority?.sha256 !== sha(authorityBytes) ||
    capture.authority?.contentHash !== portableAssetContentHash(authorityBytes)
  )
    throw new Error("capture does not bind the exact I1 authority");
  const sourcePaths = [
  ...
  if (!Array.isArray(capture.source) || capture.source.map(({ path }) => path).join(",") !== sourcePaths.join(","))
    throw new Error("capture lacks exact I1 review implementation source closure");
  for (const entry of capture.source) {
    ...
    if (sha(bytes) !== entry.sha256 || portableAssetContentHash(bytes) !== entry.contentHash)
      throw new Error(`capture I1 review implementation drifted: ${entry.path}`);
  }
  for (const key of [
    "approvedShell",
    "approvedMaterials",
    ...
    if (!exact(capture[key], authority[key])) throw new Error(`capture ${key} does not bind the exact I1 authority`);
  if (
    capture.studio?.neutral !== true ||
    capture.studio?.world !== "none" ||
    capture.studio?.fixedTimeSeconds !== authority.presentation.fixedTimeSeconds
  )
    throw new Error("capture lacks the exact neutral I1 studio contract");
  if (
  ...
  if (
    !exact(capture.mounted?.inventory, inventory) ||
    !integer(capture.mounted?.labelCount, 1) ||
    !integer(capture.lifecycle?.baselineEntities) ||
    capture.lifecycle.afterDisposeEntities !== capture.lifecycle.baselineEntities
  )
    throw new Error("capture lacks exact proxy inventory, labels, or lifecycle return evidence");
  const views = authority.evidenceViews;
  if (
    views.length !== 2 ||
    views.map(({ id }) => id).join(",") !== "layout-top-down,entry-walkthrough" ||
    !Array.isArray(capture.captures) ||
    capture.captures.length !== 2 ||
    !Array.isArray(capture.outputs) ||
    capture.outputs.length !== 2
  )
    throw new Error("capture lacks exactly the canonical two-view I1 evidence set");
  for (let index = 0; index < 2; index++) {
    ...
    if (
      ...
      record.shellVisible !== view.shellVisible ||
      record.proxiesVisible !== true ||
      output.id !== view.id ||
      output.role !== view.role ||
      output.shellVisible !== view.shellVisible ||
      output.proxiesVisible !== true
    )
      throw new Error("capture I1 views are incomplete or mislabeled");
    ...
    if (
      ...
      output.width < Math.max(1920, authority.presentation.minimumResolution[0]) ||
      output.height < Math.max(1080, authority.presentation.minimumResolution[1]) ||
      !/^sha256:[0-9a-f]{64}$/.test(output.pngSha256) ||
      !integer(output.pngByteLength, 1) ||
      !/^sha256:[0-9a-f]{64}$/.test(output.rgbaContentHash) ||
      Number.isNaN(Date.parse(output.timestamp)) ||
      output.exposureEvidence?.schema !== "limina.cpu-pixel-exposure/v1"
    )
      throw new Error(`I1 ${output.id} is not a canonical review PNG`);
  }
  if (new Set(capture.outputs.map(({ pngSha256 }) => pngSha256)).size !== 2)
    throw new Error("I1 canonical PNG evidence hashes must be distinct");
  const candidate = validateBuildingStageArtifact({
    ...
    metadata: {
      ...
      authorityPath: portable(root, authorityAbsolute),
      captureEvidencePath: portable(root, captureAbsolute),
  ...
  const args = process.argv.slice(2),
    at = (flag) => {
      ...
      if (index < 0 || !args[index + 1])
        throw new Error(
          "usage: bun tools/architecture/build-interior-review-candidate.mjs --authority <json> --capture <json> --draft <json> --out <json>",
        );
      return args[index + 1];
    };
