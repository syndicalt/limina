...
const args = process.argv.slice(2),
  at = (flag: string) => {
    ...
    if (index < 0 || !args[index + 1])
      throw new Error(
        "usage: bun tools/architecture/build-shell-v2.ts --spec <json> --out <glb> --blend-out <blend> --evidence <json>",
      );
...
const adapterPath = new URL("../blender/architecture-adapter-v2.py", import.meta.url).pathname,
  run = Bun.spawnSync(
    [
      toolchain.binary,
      "--background",
