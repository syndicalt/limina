...
const args=process.argv.slice(2),value=(flag)=>{const index=args.indexOf(flag);if(index<0||!args[index+1])throw new Error("usage: bun tools/preview/run-native-furniture-pack-capture.mjs --authority <json> --out-dir <new-private-directory>");return args[index+1];};
...
const authorityRelative=relative(repo,authorityPath),reviewRelative=relative(internalRoot,reviewRoot);
if(authorityRelative===".."||authorityRelative.startsWith(`..${sep}`)||isAbsolute(authorityRelative))throw new Error("furniture authority escaped the repository root");
...
const expectedViews = authority.evidenceViews.map(({ id, role, distanceM, ...view }) => ({ id, role, distanceM, ...view }));
const expectedAuthoritativeBounds = authority.bounds.max.map((value, index) => value - authority.bounds.min[index]);
if(expectedViews.length<1||new Set(expectedViews.map(({id})=>id)).size!==expectedViews.length||expectedViews.some(({id})=>!id||id.includes("/")||id.includes("\\")))throw new Error("furniture authority review view ids must be unique safe filenames");
try{await access(reviewRoot);throw new Error(`append-only furniture capture output already exists: ${relative(repo,reviewRoot)}`);}catch(error){if(error?.code!=="ENOENT")throw error;}
...
function runGuarded(command, args, env) {
  return new Promise((accept, reject) => {
    ...
    child.on("exit", (code, signal) => { clearInterval(poll); monitor.kill("SIGTERM"); if (guardFailure) reject(new Error(guardFailure)); else if (code !== 0) reject(new Error(`native furniture capture exited ${code ?? signal}`)); else accept(); });
  });
}

await access(binary);
const bootId = (await readFile("/proc/sys/kernel/random/boot_id", "utf8")).trim();
const preflight=kernelLog().split(/\r?\n/).find((line)=>XID.test(line));if(preflight)throw new Error(`current boot already contains an NVIDIA Xid; reboot before any native capture retry: ${preflight.trim()}`);
const captureEnv = { ...process.env, LIMINA_ASSET_ROOT: repo, LIMINA_FURNITURE_REVIEW_AUTHORITY: authorityRelative.split(sep).join("/") };
...
if(Object.keys(captureEnv).some((key)=>/TIMESTAMP/i.test(key)))throw new Error("timestamp environment variables are forbidden for furniture capture");
await unlink(tracePath).catch((error)=>{if(error?.code!=="ENOENT")throw error;});
let captureFailure;try{await runGuarded(binary, ["--window", ...(fullscreenSetting === "1" ? ["--fullscreen"] : []), "--width", String(minimumWidth), "--height", String(minimumHeight), "--frames", "1", modulePath], captureEnv);}catch(error){captureFailure=error;}
const postflight=kernelLog().split(/\r?\n/).find((line)=>XID.test(line));if(postflight){await unlink(tracePath).catch(()=>{});throw new Error(`NVIDIA Xid detected after capture; stop and reboot before any retry: ${postflight.trim()}`);}if(captureFailure){await unlink(tracePath).catch(()=>{});throw captureFailure;}

let traceBytes;try{traceBytes=await readFile(tracePath);}finally{await unlink(tracePath).catch((error)=>{if(error?.code!=="ENOENT")throw error;});}
const artifact = JSON.parse(traceBytes.toString("utf8"));
if (artifact.schema !== "limina.furniture-pack-native-review-set/v1" || artifact.backend !== "native-webgpu"
    || artifact.captureClass !== "production-engine" || artifact.pixelFormat !== "rgba8unorm" || artifact.rowOrigin !== "top-left"
    ...
    || artifact.pack?.id !== authority.pack.id || artifact.pack?.assetId !== authority.pack.assetId
    || artifact.pack?.sha256 !== authority.pack.sha256 || artifact.pack?.assetHash !== authority.pack.assetHash
    ...
    || !Array.isArray(artifact.mounted?.authoritativeBounds) || artifact.mounted.authoritativeBounds.length !== 3
    || artifact.mounted.authoritativeBounds.some((value, index) => !Number.isFinite(value) || Math.abs(value - expectedAuthoritativeBounds[index]) > 0.002)) {
  throw new Error("furniture capture identity or timestamp policy drifted");
}
if (!Array.isArray(artifact.captures) || artifact.captures.length !== expectedViews.length
    || artifact.captures.some((capture, index) => capture.id !== expectedViews[index].id
      || capture.role !== expectedViews[index].role || capture.camera?.distanceM !== expectedViews[index].distanceM
      || JSON.stringify(capture.reviewState)!==JSON.stringify({type:expectedViews[index].type,state:expectedViews[index].state,appliedState:`${expectedViews[index].type}:${expectedViews[index].state}`}))) {
  throw new Error("furniture capture review views are incomplete or mislabeled");
}
if (!Number.isSafeInteger(artifact.lifecycle?.baselineEntities) || artifact.lifecycle.baselineEntities < 0
    || artifact.lifecycle?.afterDisposeEntities !== artifact.lifecycle.baselineEntities) throw new Error("furniture capture lacks lifecycle return evidence");
...
for (const capture of artifact.captures) {
  ...
  if (capture.width < minimumWidth || capture.height < minimumHeight || capture.surfaceFormat !== artifact.surfaceFormat
      || submission?.schema !== "limina.three-render-submission/v2" || submission.source !== "three-webgpu-renderer-info"
      ...
      || !Number.isFinite(submission.cpuEncodeMs) || submission.cpuEncodeMs < 0) throw new Error(`furniture capture ${capture.id} lacks whole-frame telemetry`);
  ...
  if (resources?.schema !== "limina.three-render-resources/v1" || resources.source !== "three-webgpu-renderer-info"
      || resources.scope !== "renderer-live-after-production-frame" || resourceValues.length !== 12
      || resourceValues.some((value) => !Number.isSafeInteger(value) || value < 0)) throw new Error(`furniture capture ${capture.id} lacks renderer resource telemetry`);
  ...
  if (paired?.schema !== "limina.paired-render-submission/v1"
      ...
      || !Number.isSafeInteger(paired.delta.triangles) || paired.delta.triangles < 1) throw new Error(`furniture capture ${capture.id} lacks strict paired subject telemetry`);
  const rgba = Buffer.from(capture.rgbaBase64, "base64");
  if (rgba.length !== capture.width * capture.height * 4
      || capture.rgbaByteLength !== rgba.length || portableAssetContentHash(rgba) !== capture.rgbaContentHash) throw new Error(`furniture capture ${capture.id} has invalid pixels`);
  const output = resolve(reviewRoot, `${capture.id}.png`);
  const png=await sharp(rgba, { raw: { width: capture.width, height: capture.height, channels: 4 } }).png({ compressionLevel: 9, adaptiveFiltering: true }).toBuffer();
  await writeFile(output,png,{mode:0o600,flag:"wx"});await chmod(output, 0o600);
  outputs.push({ id: capture.id, role: capture.role, type:capture.reviewState.type, state:capture.reviewState.state, appliedState:capture.reviewState.appliedState, distanceM: capture.camera.distanceM, width: capture.width, height: capture.height,
    path: relative(repo, output), pngSha256: `sha256:${createHash("sha256").update(png).digest("hex")}`, pngByteLength: png.length,
    rgbaContentHash: capture.rgbaContentHash, renderSubmission: submission, rendererResources: resources });
}
const guardedArtifact = { ...artifact,
  captures: artifact.captures.map(({ rgbaBase64, ...capture }) => capture),
  guardEvidence: { schema: "limina.nvidia-xid-guard/v1", bootId,
