...
function fail(message) { console.error(`biome-surface-browser-gate FAIL: ${message}`); process.exit(1); }
...
window.__biomeSurface={backend:'forceWebGL',calls,mean,stdev,programs:renderer.info.programs?.length??null,pass:calls===2&&mean>2&&stdev>5};`;
try {
  await build({ stdin: { contents: entry, resolveDir: repo, sourcefile: "biome-surface-entry.ts", loader: "ts" }, bundle: true, format: "esm", outfile: join(directory, "entry.js"), logLevel: "silent" });
  ...
  try { browser = await chromium.launch({ executablePath: chrome, args: ["--no-sandbox", "--disable-dev-shm-usage", "--use-gl=angle", "--use-angle=swiftshader", "--enable-unsafe-swiftshader"] });
    const page = await browser.newPage({ viewport: { width: 384, height: 256 } }), errors = []; page.on("pageerror", (error) => errors.push(error.message)); page.on("console", (message) => { if (message.type() === "error") errors.push(message.text()); });
    await page.goto(`http://127.0.0.1:${server.address().port}/`, { waitUntil: "domcontentloaded", timeout: 15_000 });
    const result = await page.waitForFunction(() => window.__biomeSurface ?? false, { timeout: 30_000 }).then((handle) => handle.jsonValue());
    if (errors.length) fail(errors.slice(0, 6).join(" | ")); if (!result.pass) fail(`render proof failed: ${JSON.stringify(result)}`);
    console.log(`biome-surface-browser-gate OK: forceWebGL rendered two texture identities through the shared three-sample TSL graph in ${result.calls} draws; stdev ${result.stdev.toFixed(2)}`);
