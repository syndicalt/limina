...
function fail(message) {
  console.error(`water-scene-optics-browser-gate FAIL: ${message}`);
  process.exit(1);
}
...
const material=refracting.material,reflectionMaterial=reflecting.material;
window.__waterOptics={
  backend:'forceWebGL',
  changed,
  meanDelta:sum/(a.length/4),
  reflectionChanged,
  reflectionMeanDelta:reflectionSum/(a.length/4),
  refraction:material.userData.liminaWaterSceneDepthRefraction===true,
  reflection:reflectionMaterial.userData.liminaWaterPlanarReflection===true,
  reflectionTargets:reflecting.children.length,
  backdrop:material.backdropNode?.constructor?.name??null,
  pass:changed>500
    &&sum/(a.length/4)>0.5
    &&reflectionChanged>100
    &&reflectionSum/(a.length/4)>.1
    &&material.userData.liminaWaterSceneDepthRefraction===true
    &&reflectionMaterial.userData.liminaWaterPlanarReflection===true
    &&reflecting.children.length===1
};`;
try {
  await build({
    stdin: { contents: entry, resolveDir: repo, sourcefile: "water-optics-entry.ts", loader: "ts" },
    bundle: true,
    format: "esm",
  ...
  try {
    ...
    try {
      result = await page
        .waitForFunction(() => window.__waterOptics ?? false, { timeout: 30_000 })
        .then((handle) => handle.jsonValue());
    } catch (error) {
      fail(errors.length ? errors.slice(0, 6).join(" | ") : `browser result timeout: ${error.message}`);
    }
    if (errors.length) fail(errors.slice(0, 6).join(" | "));
    if (!result.pass) fail(`refraction proof failed: ${JSON.stringify(result)}`);
    console.log(
      `water-scene-optics-browser-gate OK: forceWebGL compiled depth-safe refraction + owned planar reflection; refraction changed ${result.changed} pixels (mean ${result.meanDelta.toFixed(2)}), reflection ${result.reflectionChanged} (mean ${result.reflectionMeanDelta.toFixed(2)})`,
    );
  } finally {
    await browser?.close();
