...
// coastline trace, relief/biome grouping, river/route/burg mapping, the content hash) lives in
// the pure js/src/world/fmg-map-compile.mjs, gated directly by js/test/p_fmg_compile.ts. This
// file is deliberately thin: read one file, call that function, write the result, print a
// summary.
...
function parseArgs(argv) {
  const args = { input: undefined, out: undefined, mapId: undefined, minPop: undefined };
  const positional = [];
...
if (!args.input) {
  console.error("usage: node tools/map/compile-fmg.mjs <full-export.json> [--out <path>] [--map-id <id>] [--min-pop N]");
  process.exit(1);
}
