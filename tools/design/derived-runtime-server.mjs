...
export class DerivedRuntimeServer {
  ...
  async #handle(request, response) {
    ...
    const content = CONTENT_PATH.exec(rawUrl);
    if (content !== null && method === "GET") {
      if (request.headers.range !== undefined) { this.#sendError(response, 416, "RANGE_NOT_SUPPORTED", origin); return; }
      await this.#serveContent(request, response, origin, content[1], content[2]);
      return;
    }
    if (content !== null) { this.#sendError(response, 405, "METHOD_NOT_ALLOWED", origin); return; }
    this.#sendError(response, 404, "NOT_FOUND", origin);
  }

  #handleOptions(request, response, rawUrl, origin) {
    const immutable = ARTIFACT_PATH.test(rawUrl) || CONTENT_PATH.test(rawUrl);
    if (rawUrl !== CURRENT_PATH && !immutable) { this.#sendError(response, 404, "NOT_FOUND", origin); return; }
  ...
  async #serveContent(request, response, origin, manifestHex, engineHashHex) {
    if (!HASH_HEX.test(manifestHex) || !HASH_HEX.test(engineHashHex)) { this.#sendError(response, 404, "NOT_FOUND", origin); return; }
    if (this.#activeArtifacts.size >= DERIVED_RUNTIME_MAX_TOKEN_ARTIFACT_STREAMS
        || processArtifactStreams >= DERIVED_RUNTIME_MAX_PROCESS_ARTIFACT_STREAMS) {
      this.#sendError(response, 429, "STREAM_LIMIT", origin); return;
    }
    ...
    try {
      const manifestHash = `sha256:${manifestHex}`;
      const engineHash = `sha256:${engineHashHex}`;
      let initial;
      try { initial = await this.#currentView(); }
      catch (error) { this.#publicationError(response, error, origin, false); return; }
      if (initial.manifest.manifestHash !== manifestHash) {
      ...
      const closureDescriptors = (initial.manifest.globalArtifacts ?? [])
        .filter((descriptor) => descriptor.artifactType === BIOME_CONTENT_CLOSURE_ARTIFACT_TYPE);
      if (closureDescriptors.length !== 1) { this.#sendError(response, 404, "NOT_FOUND", origin); return; }
      const closureDescriptor = closureDescriptors[0];
      if (closureDescriptor.mediaType !== BIOME_CONTENT_CLOSURE_ARTIFACT_MEDIA_TYPE
          || closureDescriptor.byteLength < 2
          || closureDescriptor.byteLength > MAX_BIOME_CONTENT_CLOSURE_ARTIFACT_BYTES) {
        this.#sendError(response, 503, "ARTIFACT_INVALID", origin); return;
      }
      let closureAccess;
      try {
        closureAccess = resolveCurrentPublishedDerivedArtifact({
          projectRoot: this.#projectRoot,
          branchId: this.#branchId,
          manifestHash,
          contentHash: closureDescriptor.contentHash,
          cachedRevision: initial,
        });
      } catch (error) {
        this.#publicationError(response, error, origin, true); return;
      ...
      let closure;
      try {
        const leaf = lstatSync(closureAccess.path);
        if (leaf.isSymbolicLink() || !leaf.isFile()) throw new Error("content closure is not a regular file");
        const handle = await open(closureAccess.path, constants.O_RDONLY | (constants.O_NOFOLLOW ?? 0));
        try {
          const opened = await handle.stat();
          if (!opened.isFile() || opened.nlink < 1 || !sameFile(leaf, opened)
              || opened.size !== closureDescriptor.byteLength) throw new Error("content closure changed while opening");
          const root = realpathSync(closureAccess.artifactRoot);
          let openedPath;
          try { openedPath = realpathSync(`/proc/self/fd/${handle.fd}`); }
          catch {
            openedPath = realpathSync(closureAccess.path);
            if (!sameFile(opened, statSync(openedPath))) throw new Error("content closure path changed after open");
          }
          if (!contained(root, openedPath) || openedPath === root) throw new Error("content closure resolved outside publication root");
          const bytes = new Uint8Array(opened.size);
          const rawHash = createHash("sha256");
          let position = 0;
          while (position < bytes.byteLength) {
            if (request.destroyed || response.destroyed || this.#abort.signal.aborted) throw new Error("content request aborted");
            const { bytesRead } = await handle.read(bytes, position, bytes.byteLength - position, position);
            if (bytesRead === 0) throw new Error("content closure truncated during validation");
            rawHash.update(bytes.subarray(position, position + bytesRead));
            position += bytesRead;
          }
          if (`sha256:${rawHash.digest("hex")}` !== closureDescriptor.contentHash) throw new Error("content closure hash mismatch");
          const finalStat = await handle.stat();
          if (!sameFile(opened, finalStat) || finalStat.nlink < 1 || finalStat.size !== opened.size) {
            throw new Error("content closure changed during validation");
          }
          closure = decodeBiomeContentClosureArtifact(bytes, { shouldCancel: () => this.#abort.signal.aborted });
          if (closure.metadata.contentHash !== closureDescriptor.contentHash
              || closure.metadata.byteLength !== closureDescriptor.byteLength) throw new Error("content closure metadata mismatch");
        } finally { await handle.close().catch(() => {}); }
      } catch {
        this.#sendError(response, 503, "ARTIFACT_INVALID", origin); return;
      }

      const entry = closure.bundle.entries.find((candidate) => candidate.contentHash === engineHash);
      if (entry === undefined) { this.#sendError(response, 404, "NOT_FOUND", origin); return; }
      if (entry.byteLength > MAX_DERIVED_ARTIFACT_BYTES) {
        this.#sendError(response, 503, "ARTIFACT_INVALID", origin); return;
      }

      const contentRoot = join(dirname(closureAccess.artifactRoot), "content");
      const path = join(contentRoot, `${engineHashHex}.bin`);
      const contentRootLeaf = lstatSync(contentRoot);
      if (contentRootLeaf.isSymbolicLink() || !contentRootLeaf.isDirectory()) throw new Error("publication content root is not a real directory");
      const branchRoot = realpathSync(dirname(contentRoot));
      const root = realpathSync(contentRoot);
      if (!contained(branchRoot, root) || root === branchRoot) throw new Error("publication content root resolved outside its branch");
      const leaf = lstatSync(path);
      if (leaf.isSymbolicLink() || !leaf.isFile()) throw new Error("content is not a regular file");
      const handle = await open(path, constants.O_RDONLY | (constants.O_NOFOLLOW ?? 0));
      active.handle = handle;
      const opened = await handle.stat();
      if (!opened.isFile() || opened.nlink < 1 || !sameFile(leaf, opened) || opened.size !== entry.byteLength) {
        throw new Error("content changed while opening");
      }
      let openedPath;
      try { openedPath = realpathSync(`/proc/self/fd/${handle.fd}`); }
      catch {
        openedPath = realpathSync(path);
        if (!sameFile(opened, statSync(openedPath))) throw new Error("content path changed after open");
      }
      if (!contained(root, openedPath) || openedPath === root) throw new Error("content resolved outside publication content root");
      ...
      while (position < opened.size) {
        if (request.destroyed || response.destroyed || this.#abort.signal.aborted) throw new Error("content request aborted");
        const { bytesRead } = await handle.read(buffer, 0, Math.min(buffer.byteLength, opened.size - position), position);
        if (bytesRead === 0) throw new Error("content truncated during validation");
        validationHash.update(buffer.subarray(0, bytesRead).toString("hex"), "utf8");
        position += bytesRead;
      }
      if (`sha256:${validationHash.digest("hex")}` !== engineHash) throw new Error("content engine hash mismatch");
      const afterHash = await handle.stat();
      if (!sameFile(opened, afterHash) || afterHash.nlink < 1 || afterHash.size !== opened.size) {
      ...
      if (confirmed.manifest.manifestHash !== manifestHash || confirmed.generation !== initial.generation
          || confirmed.generation !== closureAccess.generation) {
        this.#sendError(response, 412, "CURRENT_CHANGED", origin); return;
      }
      const etag = `"${engineHash}"`;
      const headers = {
        ...this.#corsHeaders(origin),
        ...
        "X-Limina-Manifest-Hash": manifestHash,
        "X-Limina-Generation": String(initial.generation),
      };
      if (request.headers["if-none-match"] === etag) {
        response.writeHead(304, { ...headers, "Content-Length": "0" });
        response.end();
      ...
      for await (const chunk of stream) {
        if (request.destroyed || response.destroyed || this.#abort.signal.aborted) throw new Error("content request aborted");
        streamedHash.update(chunk.toString("hex"), "utf8");
        streamedBytes += chunk.byteLength;
        if (!response.write(chunk)) await waitForDrain(response);
      }
      const finalStat = await handle.stat();
      if (streamedBytes !== entry.byteLength || `sha256:${streamedHash.digest("hex")}` !== engineHash
          || !sameFile(opened, finalStat) || finalStat.nlink < 1 || finalStat.size !== opened.size) {
        throw new Error("content changed during stream");
      }
      response.end();
    } catch {
      if (headersSent || response.headersSent) response.destroy();
