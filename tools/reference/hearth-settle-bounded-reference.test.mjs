...
test("bounded v3 is append-only and preserves every legacy hearth-settle authority byte", async () => {
  assert.equal(await sha256(legacyAcquisitionPath), "sha256:d33c7b2194b0a48ae25951415c01162e463eed598d208b73c57d1bdc0a67ab71");
  assert.equal(await sha256("art-direction/furniture/hearth-settle-reference-board.json"), "sha256:b96358f6c0b3e6b3227f8f53d117cb0b8d36ef205317da10e1bdc983897ed090");
  assert.equal(await sha256("art-direction/furniture/hearth-settle-visual-design.json"), "sha256:e995171f25df06a25946751d04ec8a207a4f982037f6a363923f2ca412e7b0ef");
  assert.equal(await sha256("art-direction/furniture/hearth-settle-v2-visual-design.json"), "sha256:2cf7ea37ce8f466f56ea3671683e78e240375dd1b8f9e14acdd6a26b459f11ac");
  const legacy = await read(legacyAcquisitionPath), bounded = await read(acquisitionPath);
...
test("reference acquisition requests original bytes and preserves stable retrieval authority", async () => {
  ...
  assert.match(source, /previousSources=new Map/);
  assert.match(source, /record\.retrievedAt=previous\.retrievedAt/);
});

test("bounded board pins both official Met 7390 Open Access views", async () => {
  const acquisition = await read(acquisitionPath), board = await read(boardPath), legacyBoard = await read("art-direction/furniture/hearth-settle-reference-board.json");
  assert.equal(board.id, "furniture/hearth-settle/v3");
  assert.equal(board.acquisitionManifest, acquisitionPath);
  assert.deepEqual(board.sources.map(({ id }) => id), [
  ...
  assert.ok(met7390.every(({ sourceUrl, license }) => sourceUrl === "https://www.metmuseum.org/art/collection/search/7390" && license === "Public Domain / The Met Open Access"));
  assert.deepEqual(met7390.map(({ imageUrl }) => imageUrl), [
...
test("v3 visual authority locks the exact I1 r3 envelope and audited cue floor", async () => {
  const board = await read(boardPath), visual = validateVisualDesignContract(await read(designPath));
  assert.equal(visual.id, "furniture/hearth-settle/v3");
  assert.deepEqual(visual.references.map(({ id }) => id), board.sources.map(({ id }) => id));
  for (const reference of visual.references) {
    ...
    if (reference.id !== "met/high-backed-chair-471833") assert.deepEqual(reference.roles, source.roles, `${reference.id}.roles is not pinned to the board`);
  }
  assert.deepEqual(visual.references.find(({ id }) => id === "met/high-backed-chair-471833").roles, ["anti-cue", "silhouette"]);
  assert.deepEqual(visual.references.find(({ id }) => id === "met/leather-settle-7390-primary").roles, ["silhouette", "construction", "function"]);
  const cues = new Map(visual.cues.map((cue) => [cue.id, new Map(cue.measurements.map((measurement) => [measurement.name, measurement]))]));
  assert.deepEqual(cues.get("locked-i1-r3-envelope-and-axis").get("overall-width"), { name: "overall-width", unit: "m", target: 1.6, maximum: 1.6 });
  assert.deepEqual(cues.get("locked-i1-r3-envelope-and-axis").get("overall-height"), { name: "overall-height", unit: "m", target: 1.3, maximum: 1.3 });
  assert.deepEqual(cues.get("locked-i1-r3-envelope-and-axis").get("overall-depth"), { name: "overall-depth", unit: "m", target: 0.7, maximum: 0.7 });
  assert.deepEqual(cues.get("dimensioned-continuous-two-adult-seat").get("usable-seat-width"), { name: "usable-seat-width", unit: "m", minimum: 1.28, target: 1.33, maximum: 1.38 });
  ...
  assert.deepEqual(cues.get("supported-arms").get("arm-top-height"), { name: "arm-top-height", unit: "m", minimum: 0.68, target: 0.69, maximum: 0.7 });
