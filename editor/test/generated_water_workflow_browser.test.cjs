...
(async () => {
  ...
  if (
    !/^wss?:\/\/(?:localhost|127\.0\.0\.1):[1-9][0-9]{0,4}\/$/.test(host) ||
  ...
  try {
    ...
    await page.evaluate(() => {
      ...
      new MutationObserver(() => window.__liminaGeneratedWaterStatuses.push(status?.textContent || "")).observe(
        status,
        { childList: true, subtree: true, characterData: true },
      );
    });
    const currentBodyPromise = page
      .waitForResponse(
        (response) => response.url() === new URL("v1/derived/current", derivedUrl).href && response.status() === 200,
        { timeout: 60_000 },
      )
    ...
    await page.waitForFunction(
      () => window.__liminaGeneratedWaterStatuses?.some((value) => /^derived: r\d+ · sha256:/.test(value)),
      null,
      { timeout: 60_000 },
    );
    const current = await currentBodyPromise;
      ...
      const probe = {
        ...
        snapshot() {
          return [...fragments.values()]
            .filter((entry) => entry.mesh.parent !== null)
            .map((entry) => ({
        ...
        async captureWaterDelta() {
          ...
          let withoutWater;
          try {
            withoutWater = capture();
          } finally {
            for (const entry of active) entry.mesh.visible = true;
          }
          renderer.render(scene, camera);
    ...
    assert.ok(
      cinematic.every((entry, index) => entry.geometry !== performance[index].geometry),
    ...
    assert.ok(
      pixels.visible > pixels.samples * 0.5 && pixels.colors >= 8,
    ...
    assert.deepEqual(
      [...new Set(teardown.removed)].sort(),
      [...new Set(teardown.added)].sort(),
    ...
    console.log(
      `generated_water_workflow_browser.test OK: exact hydrology globals, ${performance.length} Performance fragments, ${waterPixels.changed} water-changed pixels, ${waterPixels.temporalChanged} animated pixels, Cinematic rebuild, bounded teardown; ${screenshot}`,
    );
  } finally {
    await page
      ?.evaluate(async () => {
