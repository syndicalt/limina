...
(async () => {
  ...
  try {
    ...
    await atlas.waitForFunction(() => document.getElementById("projname")?.textContent !== "Design Space", null, { timeout: 15_000 });
    ...
    await editor.waitForURL((url) => url.origin === new URL(BASE_URL).origin && url.pathname === "/", { timeout: 30_000 });
    ...
    if (!landing.openerNull || landing.pending !== null || !landing.isolated
        || landing.server !== EXPECTED_HANDOFF_SERVER || /token|focus|nonce|headHash/i.test(landing.href)) {
    ...
    try {
      await editor.waitForFunction(() => !document.getElementById("viewport-navigation-views-toggle")?.disabled, null, { timeout: 45_000 });
    } catch {
    ...
    await editor.locator("#viewport-navigation-recents").filter({ hasText: /Grey Field|primary/i }).waitFor({ timeout: 45_000 });
    ...
    if (after.head.headHash !== before.head.headHash || after.head.revision !== before.head.revision) {
      fail(`standalone handoff mutated authority: before=${before.head.revision}/${before.head.headHash} after=${after.head.revision}/${after.head.headHash}`);
    }
    ...
    const rejectedPopupPromise = context.waitForEvent("page", { timeout: 10_000 });
    await proxied.locator("#open-editor").click();
    const rejectedPopup = await rejectedPopupPromise;
    await proxied.locator("#ds-toast").filter({ hasText: /Atlas solo URL/ }).waitFor({ timeout: 10_000 });
    await rejectedPopup.waitForEvent("close", { timeout: 10_000 }).catch(() => {});
    if (!rejectedPopup.isClosed() && new URL(rejectedPopup.url()).origin === new URL(BASE_URL).origin) {
      fail(`proxied Atlas reached the editor handoff: ${rejectedPopup.url()}`);
    }
    await proxied.close();
    if (!rejectedPopup.isClosed()) await rejectedPopup.close();
    console.log("atlas_standalone_handoff_browser.test OK: direct relay, one-shot exact focus, proxied rejection, authority unchanged");
  } finally {
    authority.close();
    await context.close();
    await browser.close();
