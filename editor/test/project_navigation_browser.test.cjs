...
(async () => {
  ...
  const goTo = async (target) => {
    ...
    await page.waitForFunction(() => document.getElementById("viewport-navigation-orbit")?.disabled
  ...
  try {
    ...
    assert.ok(Object.values(initialTarget).every(Number.isFinite), `initial target is invalid: ${JSON.stringify(initialTarget)}`);
    await closeGoto();
    ...
    await page.waitForFunction(
      () => [...document.querySelectorAll("#viewport-navigation-bookmarks [data-navigation-entry]")]
        .some((button) => button.textContent === "UAT initial view"),
    );
    ...
    assert.ok(flownSignal.visible > flownSignal.sampled * 0.5 && flownSignal.colors >= 4,
    ...
    await page.waitForFunction(() => document.getElementById("viewport-navigation-views")?.hidden === true, null, { timeout: 30_000 });
    const bookmarkTarget = await readTarget();
    assert.ok(distance(initialTarget, bookmarkTarget) < 1e-6,
      `bookmark did not restore the exact initial target: ${JSON.stringify({ initialTarget, bookmarkTarget })}`);
    assert.equal(await page.getAttribute("#viewport-navigation-orbit", "aria-checked"), "true",
    ...
    const farTarget = { x: 1200, y: initialTarget.y, z: 1200 };
    ...
    assert.ok(destinationSignal.visible > destinationSignal.sampled * 0.5 && destinationSignal.colors >= 4,
    ...
    await page.click("#viewport-navigation-views-toggle");
    assert.ok(await page.locator("#viewport-navigation-recents [data-navigation-entry]").count() >= 2,
    ...
    await page.waitForFunction(
      () => document.getElementById("viewport-play-state")?.textContent === "Playing",
      null,
      { timeout: 30_000 },
    );
    await page.click("#viewport-stop");
    await page.waitForFunction(
      () => document.getElementById("viewport-play-state")?.textContent === "Edit",
      null,
      { timeout: 30_000 },
    );
    const restoredTarget = await readTarget();
    assert.ok(distance(initialTarget, restoredTarget) < 1e-6,
    ...
    assert.ok(restoredSignal.visible > restoredSignal.sampled * 0.5 && restoredSignal.colors >= 4,
    ...
    console.log(`project_navigation_browser.test OK: Fly moved ${distance(initialTarget, flownTarget).toFixed(1)}m; `
