...
| **W3 — Agent control + coarse→fine** | ~~Steerable coarse continent/biome guide via `generateRegion` hints, detailed by noise + erosion.~~ **RE-SCOPED 2026-07-07 — largely superseded.** The "agent sketches intent, the generator builds it" model now ships for the *authored-map* path: the Map Painter (`plans/map-painter.md`) + WorldMap IR compiler + `terrain.create source:"map"` let an agent (or human) paint landmass/elevation/biome/waterway/stamp intent and stream it into a real world, deterministically, with a real-GPU peek. What remains of W3 is only the *pure-procedural* hint API (steer a noise-generated region without painting it) — a thin convenience over the same IR the painter already emits; deprioritized behind W2 erosion, which benefits every path. | none | low |
