...
Status: **FB-2 CLOSED; FB-3 COMPLETE; FB-4 MECHANICALLY CLOSED / VISUAL HITL PENDING (2026-07-16).** The Blender-authored R1 cottage
...
multi-room authority; it does not reinterpret the approved v1 artifact. Its first native visual
candidate was mechanically valid but centrally rejected, and the replacement now includes the
program-first synthesis migration described in
...
3. **FB-3 — occupancy and interaction (complete).** Deterministic occupied-close rule (block and remain open),
   ...
   doors.
4. **FB-4 — multi-room semantics.** Multiple rooms/portals, stairs and upper floors, room-aware audio,
   visibility/streaming cells, interior spawn anchors, and traversal tests across every portal.
5. **FB-5 — generated settlements.** Contract-aware building library, variants/LODs that preserve
...
FB-5 publication authority is now being built behind the still-open FB-4 visual gate. The strict catalog
...
site/foundation and furnishing references, and whole-building residency units. Atlas resolution retains
...
composition, and settlement lifecycle integration remain active work; none of these pure authorities permits
...
is `fb4-multi-room-candidate-d0ca1e327841`; it closes both compiler-owned gable ends, fulfills all eight
program-required windows, and derives 150 aperture-clear, connected, perceptual-only exterior frame members
from final shell authority. Its manifest binds the full 54-file production and material-source closure so
...
directions at large world coordinates and arbitrary yaw. The additive rendered review artifact and its exact
owner decision remain open; no earlier approval applies.
...
The runner retains the boot/live/postflight NVIDIA Xid stop, removes timestamp-risk environment
